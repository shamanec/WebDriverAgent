...
@interface FBMjpegServerGads()

@property (nonatomic, readonly) dispatch_queue_t backgroundQueue;
@property (nonatomic, readonly) NSMutableArray<GCDAsyncSocket *> *listeningClients;
@property (nonatomic, readonly) FBImageProcessorGads *imageProcessor;
...
@implementation FBMjpegServerGads

static NSData *previousScreenshotData;
static uint32_t previousScreenshotHash;
static NSUInteger previousScreenshotSize;
...
- (instancetype)init
{
  if ((self = [super init])) {
    ...
    _imageProcessor = [[FBImageProcessorGads alloc] init];
...
- (void)streamScreenshot
{
  ...
  // Fast hash-based frame comparison instead of expensive NSData comparison
  NSUInteger currentSize = screenshotData.length;
  uint32_t currentHash = (uint32_t)crc32(0, screenshotData.bytes, (uInt)currentSize);
  ...
  [self.imageProcessor submitImageData:screenshotData
    ...
    previousScreenshotSize = currentSize;
